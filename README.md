...
- **Assisted** (`--assist`): an external model (codex CLI if available, otherwise
  Claude Code) pre-labels each pair. You review with Enter-to-accept for obvious
  cases, override only when you disagree. Cuts labeling time from ~15 min to ~3 min.
- **Batch** (`--batch answers.json`): fully non-interactive, reads pre-computed
  labels from a JSON file. For CI and repeat runs.

Use `--dry-run` to preview which excerpt+dimension pairs the relevance filter
will present before committing to a labeling session.
...
**Cross-model generation.** To reduce same-model bias (the judge evaluating text
its own model wrote), synthetic examples and mutations can be generated by a
different model. If `codex` CLI is available, Claude Code uses it for content
generation while keeping judging for itself. Generate one dimension at a time
(~20 examples per call) to avoid timeouts with external tools.
...
**Score caching.** After an adoption, before-scores are cached and reused in
subsequent iterations until the next adoption or adversarial pass. This
eliminates judge variance on unchanged artifacts -- the same text won't get
different verdicts across runs. Use `--force-rejudge` to bypass the cache.

**Stall detection.** After 3+ consecutive discards, a score plateau, or an
all-PASS ceiling (every dimension passing but mutations keep getting discarded),
the system warns you. At the all-PASS ceiling, it surfaces adversarial findings
as candidates for new dimensions. Without this, the loop burns tokens on
mutations that will never land.
...
into the next mutation and persist in `runs/adversarial_log.jsonl` so the
morning report can show all findings across iterations, not just the latest.
...
After an adoption, the verdict saves all dimension scores to
`state['cached_scores']`. Next `score-before` checks: if cached scores exist
and last verdict was DISCARDED (same artifact), cached scores are written
directly as `judge_response.jsonl` -- no re-judging needed. Cache invalidates
on adoption (artifact changed), adversarial pass (context shifted), or
`--force-rejudge`.

### Stall detection

Three independent signals, any triggers a warning:
- Consecutive discards: N in a row (default threshold: 3)
- Score plateau: identical `composite_before` for N consecutive iterations
- All-PASS ceiling: every dimension passing + mutations keep getting discarded.
  Surfaces adversarial findings as candidates for new dimensions
...
| Cross-model generation | Single model for everything | Avoids same-model bias (judge evaluating its own writing) |
| Score caching | Re-judge every iteration | Eliminates variance on unchanged artifacts |
| Assisted labeling | Interactive-only | Cuts labeling time ~80% while preserving human override signal |
